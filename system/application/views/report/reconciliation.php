<?php
	...
	if ( ! $print_preview)
	{
		...
		if ($reconciliation_type == 'all')
			$config['total_rows'] = (int)$this->db->from('vouchers')->join('entry_items', 'vouchers.id = entry_items.voucher_id')->where('entry_items.ledger_id', $ledger_id)->count_all_results();
		else
			$config['total_rows'] = (int)$this->db->from('vouchers')->join('entry_items', 'vouchers.id = entry_items.voucher_id')->where('entry_items.ledger_id', $ledger_id)->where('entry_items.reconciliation_date', NULL)->count_all_results();
	...
	if ($ledger_id != 0)
	{
		...
		$this->db->select_sum('amount', 'drtotal')->from('entry_items')->join('vouchers', 'vouchers.id = entry_items.voucher_id')->where('entry_items.ledger_id', $ledger_id)->where('entry_items.dc', 'D')->where('entry_items.reconciliation_date IS NOT NULL');
		...
		$this->db->select_sum('amount', 'crtotal')->from('entry_items')->join('vouchers', 'vouchers.id = entry_items.voucher_id')->where('entry_items.ledger_id', $ledger_id)->where('entry_items.dc', 'C')->where('entry_items.reconciliation_date IS NOT NULL');
		...
		if ( ! $print_preview)
		{
			$this->db->select('vouchers.id as vid, vouchers.number as vnumber, vouchers.date as vdate, vouchers.narration as vnarration, vouchers.voucher_type as vtype, entry_items.id as lid, entry_items.amount as lamount, entry_items.dc as ldc, entry_items.reconciliation_date as lreconciliation');
			if ($reconciliation_type == 'all')
				$this->db->from('vouchers')->join('entry_items', 'vouchers.id = entry_items.voucher_id')->where('entry_items.ledger_id', $ledger_id)->order_by('vouchers.date', 'asc')->order_by('vouchers.number', 'asc')->limit($pagination_counter, $page_count);
			else
				$this->db->from('vouchers')->join('entry_items', 'vouchers.id = entry_items.voucher_id')->where('entry_items.ledger_id', $ledger_id)->where('entry_items.reconciliation_date', NULL)->order_by('vouchers.date', 'asc')->order_by('vouchers.number', 'asc')->limit($pagination_counter, $page_count);
			$ledgerst_q = $this->db->get();
		} else {
			$page_count = 0;
			$this->db->select('vouchers.id as vid, vouchers.number as vnumber, vouchers.date as vdate, vouchers.narration as vnarration, vouchers.voucher_type as vtype, entry_items.id as lid, entry_items.amount as lamount, entry_items.dc as ldc, entry_items.reconciliation_date as lreconciliation');
			if ($reconciliation_type == 'all')
				$this->db->from('vouchers')->join('entry_items', 'vouchers.id = entry_items.voucher_id')->where('entry_items.ledger_id', $ledger_id)->order_by('vouchers.date', 'asc')->order_by('vouchers.number', 'asc');
			else
				$this->db->from('vouchers')->join('entry_items', 'vouchers.id = entry_items.voucher_id')->where('entry_items.ledger_id', $ledger_id)->where('entry_items.reconciliation_date', NULL)->order_by('vouchers.date', 'asc')->order_by('vouchers.number', 'asc');
			$ledgerst_q = $this->db->get();
		}
