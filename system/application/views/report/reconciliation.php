<?php
	...
	if ($ledger_id != 0)
	{
		...
		foreach ($ledgerst_q->result() as $row)
		{
			...
			echo "</td>";
			echo "<td>";
			echo anchor('entry/view/' . $current_voucher_type['label'] . '/' . $row->vid, full_voucher_number($row->vtype, $row->vnumber), array('title' => 'View ' . $current_voucher_type['name'] . ' Entry', 'class' => 'anchor-link-a'));
			echo "</td>";
