<?php
	...
	if ( ! $print_preview)
	{
		...
		if ($reconciliation_type == 'all')
			...
			$config['total_rows'] = (int)$this->db->from('vouchers')->join('entry_items', 'vouchers.id = entry_items.entry_id')->where('entry_items.ledger_id', $ledger_id)->where('entry_items.reconciliation_date', NULL)->count_all_results();
	...
	if ($ledger_id != 0)
	{
		...
		if ( ! $print_preview)
		{
			$this->db->select('vouchers.id as vid, vouchers.number as vnumber, vouchers.date as vdate, vouchers.narration as vnarration, vouchers.entry_type as vtype, entry_items.id as lid, entry_items.amount as lamount, entry_items.dc as ldc, entry_items.reconciliation_date as lreconciliation');
			if ($reconciliation_type == 'all')
				...
				$this->db->from('vouchers')->join('entry_items', 'vouchers.id = entry_items.entry_id')->where('entry_items.ledger_id', $ledger_id)->where('entry_items.reconciliation_date', NULL)->order_by('vouchers.date', 'asc')->order_by('vouchers.number', 'asc')->limit($pagination_counter, $page_count);
		...
		foreach ($ledgerst_q->result() as $row)
		{
			...
			echo "</td>";
			echo "<td>";
			echo anchor('voucher/view/' . $current_entry_type['label'] . '/' . $row->vid, full_entry_number($row->vtype, $row->vnumber), array('title' => 'View ' . $current_entry_type['name'] . ' Entry', 'class' => 'anchor-link-a'));
			echo "</td>";
