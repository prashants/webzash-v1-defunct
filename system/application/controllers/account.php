...
class Account extends Controller {
	...
	function index()
	{
		...
		$data['account_list'] .= "<thead><tr><th>Name</th><th>Type</th><th>O/P Balance</th><th colspan=2>Actions</th></tr></thead>";
	...
	function _get_groups($group_id)
	{
		...
		if ($ledger_q->num_rows() > 0)
		{
			...
			foreach ($ledger_q->result() as $row)
			{
				...
				$this->account_list .= "</td>";
				$this->account_list .= "<td>LEDGER</td>";
				$this->account_list .= "<td align=\"right\">" . convert_dc($row->op_balance_dc) . " " . $row->op_balance . "</td>";
