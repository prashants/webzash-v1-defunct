...
class Group extends Controller {
	...
	function edit($id)
	{
		...
		if ($id <= 4) {
			$this->messages->add('Cannot edit System Group accounts.', 'error');
			redirect('account');
			return;
		}
	...
	function delete($id)
	{
		...
		{
			$this->messages->add('Cannot delete non-empty Group account. Sub-Groups present.', 'error');
			redirect('account');
			return;
		}
		...
		if ($this->db->get()->num_rows() > 0)
		{
			$this->messages->add('Cannot delete non-empty Group account. Sub-Ledgers present.', 'error');
			redirect('account');
			return;
		}
